...
class FhevmProviderExtender extends plugins_1.ProviderWrapper {
    ...
    async request(args) {
        switch (args.method) {
            ...
            case mock_utils_1.relayer.RELAYER_V1_DELEGATED_USER_DECRYPT:
    ...
    async _handleFhevmRelayerV1UserDecrypt(args) {
        ...
        await mock_utils_1.MockFhevmInstance.verifyUserDecryptSignature(payload.publicKey, payload.signature, payload.contractAddresses, payload.userAddress, Number(payload.requestValidity.startTimestamp), Number(payload.requestValidity.durationDays), fhevmEnv.getGatewayDecryptionAddress(), Number(payload.contractsChainId));
    ...
    async _handleFhevmRelayerV1DelegatedUserDecrypt(args) {
        const fhevmEnv = EnvironmentExtender_1.fhevmContext.get();
        if (!fhevmEnv.useEmbeddedMockEngine) {
            return this._wrappedProvider.request(args);
        }
        ...
        _assertSingleParamArray(args);
        const payload = args.params[0];
        mock_utils_1.relayer.assertIsRelayerV1DelegatedUserDecryptPayload(payload);
        await mock_utils_1.MockFhevmInstance.verifyUserACLPermissions(this._provider, fhevmEnv.getACLAddress(), payload.handleContractPairs, payload.delegatorAddress);
        await mock_utils_1.MockFhevmInstance.verifyDelegatedUserDecryptSignature({
        ...
        await mock_utils_1.MockFhevmInstance.verifyUserDecryptionDelegation(this._provider, fhevmEnv.getACLAddress(), payload.delegatorAddress, payload.delegateAddress, payload.handleContractPairs);
        const handleBytes32HexList = payload.handleContractPairs.map((h) => {
            return ethers_1.ethers.toBeHex(ethers_1.ethers.toBigInt(h.handle), 32);
        });
        const clearTextHexList = await fhevmEnv.coprocessor.queryHandlesBytes32AsHex(handleBytes32HexList);
